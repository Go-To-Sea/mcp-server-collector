...
REQUEST_TIMEOUT = int(os.getenv("REQUEST_TIMEOUT", "30"))  # 增加默认超时时间到30秒
MAX_RETRIES = int(os.getenv("MAX_RETRIES", "5"))  # 增加最大重试次数
MIN_RETRY_DELAY = float(os.getenv("MIN_RETRY_DELAY", "2.0"))  # 最小重试延迟
MAX_RETRY_DELAY = float(os.getenv("MAX_RETRY_DELAY", "60.0"))  # 最大重试延迟
...
def request_with_retry(url, headers, proxies=None, max_retries=MAX_RETRIES, timeout=REQUEST_TIMEOUT):
    ...
    for attempt in range(max_retries):
        try:
            # 采用指数退避策略，而不是简单的随机延迟
            if attempt > 0:
                # 指数退避: 基础延迟 * (2^尝试次数) + 随机化
                delay = MIN_RETRY_DELAY * (2 ** (attempt - 1)) + random.uniform(0, 1)
                delay = min(delay, MAX_RETRY_DELAY)  # 限制最大延迟时间
                print(f"⏳ 等待 {delay:.2f} 秒后进行第 {attempt+1}/{max_retries} 次重试...")
                time.sleep(delay)
            ...
            print(f"📥 收到响应: 状态码={response.status_code}, 用时={elapsed:.2f}秒")
...
def get_readme_content(repo_full_name):
    ...
    for readme_format in readme_formats:
        ...
        try:
            print(f"📄 尝试获取 {repo_full_name} 的 {readme_format}...")
            ...
            if response and response.status_code == 200:
                ...
                if 'content' in data and data.get('encoding') == 'base64':
                    ...
                    print(f"✅ 成功获取 {repo_full_name} 的 {readme_format}!")
                    return content
            elif response and response.status_code == 404:
                print(f"ℹ️ {repo_full_name} 不存在 {readme_format} 文件")
            elif response:
                print(f"⚠️ 获取 {readme_format} 失败，状态码: {response.status_code}")
        except Exception as e:
            print(f"❌ 获取 {repo_full_name} 的 {readme_format} 时出错: {e}")
            continue
    
    print(f"⚠️ {repo_full_name} 未找到任何README文件")
    ...
    for year in range(END_YEAR, START_YEAR - 1, -1):
        ...
        for month in range(month_end, month_start - 1, -1):
            ...
            while True:
                ...
                print(f"⏱️ 休息 {wait_time:.2f} 秒后继续下一批请求...")
